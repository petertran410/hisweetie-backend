...
export class ClientUserService {
  ...
  async publicConfirmOrderReceived(
    ...
    if (order.phone !== phone) {
      throw new BadRequestException('Phone number does not match order');
    }

    const expectedOrderCode = order.order_kiot_code || `DH${orderId}`;
    if (orderCode !== expectedOrderCode) {
      throw new BadRequestException('Order code does not match');
    }

    if (!order.order_kiot_id || !order.order_kiot_code) {
      throw new BadRequestException(
        'Order has not been synced to KiotViet. Cannot verify invoice.',
      );
    }
    ...
    if (order.status !== 'DELIVERED') {
      throw new BadRequestException(
        'Can only confirm received for delivered orders',
      );
    }

    try {
      const verification = await this.kiotVietService.verifyOrderDelivered(
        order.order_kiot_id,
      );

      if (!verification.isDelivered) {
        throw new BadRequestException(
          `Invoice not confirmed as delivered on KiotViet. Delivery status: ${verification.deliveryStatus}`,
        );
      }

      const kiotOrderData = verification.orderData;
      const orderTotalMatch =
        Math.abs(Number(order.total) - Number(kiotOrderData.total)) < 1;

      if (!orderTotalMatch) {
        throw new BadRequestException(
          'Order amount mismatch between database and KiotViet invoice',
        );
      }
      ...
      await this.prisma.product_order.update({
        ...
        data: {
          ...
          updated_date: new Date(),
          updated_by: 'PUBLIC_API_CONFIRMATION',
        },
      });
      ...
      await this.prisma.payment_logs.create({
        data: {
          order_id: order.id,
          event_type: 'CUSTOMER_CONFIRM_RECEIVED_PUBLIC',
          event_data: {
            confirmedAt: new Date().toISOString(),
            previousStatus: 'DELIVERED',
            newStatus: 'CUSTOMER_RECEIVED',
            verificationMethod: 'API_KEY',
            kiotVietVerification: {
              orderId: order.order_kiot_id,
              orderCode: order.order_kiot_code,
              deliveryStatus: verification.deliveryStatus,
              isDelivered: verification.isDelivered,
              hasInvoice: verification.hasInvoice,
            },
          },
          created_date: new Date(),
          ip_address: 'PUBLIC_API',
          user_agent: 'EXTERNAL_SYSTEM',
        },
      });
      ...
      return {
        ...
        status: 'CUSTOMER_RECEIVED',
        verification: {
          kiotVietOrderId: order.order_kiot_id,
          kiotVietOrderCode: order.order_kiot_code,
          deliveryConfirmed: verification.isDelivered,
        },
      };
    } catch (error) {
      if (
      ...
      throw new BadRequestException(
        `Failed to verify invoice with KiotViet: ${error.message}`,
      );
    }
  }
}
