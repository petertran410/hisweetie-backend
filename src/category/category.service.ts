...
export class CategoryService {
  ...
  async createCustomCategory(createCategoryDto: CreateCategoryDto) {
    try {
      ...
      this.logger.log(
        `✅ Created custom category: ${category.name} (ID: ${category.id})`,
      );
  ...
  /**
   * Update custom category with hierarchy validation
   */
    ...
    try {
      ...
      if (updateData.parent_id !== undefined) {
        ...
        if (updateData.parent_id) {
          const parentCategory = await this.prisma.category.findUnique({
            where: { id: BigInt(updateData.parent_id) },
          });

          if (!parentCategory) {
            throw new BadRequestException(
              `Parent category with ID ${updateData.parent_id} not found`,
            );
          }
          ...
          if (isCircular) {
            throw new BadRequestException(
              'Creating this parent relationship would create a circular reference',
            );
          }
        }
      }
      ...
      if (updateData.name && updateData.name !== existingCategory.name) {
        ...
        if (duplicateCategory) {
          throw new BadRequestException(
            `Category with name "${updateData.name}" already exists at this level`,
          );
        }
      }
      ...
      if (updateData.priority !== undefined)
        updateInput.priority = updateData.priority;
      if (updateData.parent_id !== undefined) {
        updateInput.parent_id = updateData.parent_id
          ? BigInt(updateData.parent_id)
          : null;
      }
      ...
      this.logger.log(
        `✅ Updated category: ${updatedCategory.name} (ID: ${id})`,
      );
  ...
  /**
   * Delete custom category with safety checks
   */
  async deleteCustomCategory(id: number) {
    try {
      ...
      if (categoryToDelete._count.product > 0) {
        throw new BadRequestException(
          `Cannot delete category "${categoryToDelete.name}" because it has ${categoryToDelete._count.product} associated products. Please reassign or remove products first.`,
        );
      }
      ...
      if (childCategories.length > 0) {
        throw new BadRequestException(
          `Cannot delete category "${categoryToDelete.name}" because it has ${childCategories.length} child categories. Please reassign or delete child categories first.`,
        );
      }
      ...
      this.logger.log(
        `✅ Deleted category: ${categoryToDelete.name} (ID: ${id})`,
      );
  ...
  private async checkCircularReference(
    ...
    while (currentParentId) {
      ...
      currentParentId = parent?.parent_id ? Number(parent.parent_id) : null;
    }
  ...
  /**
   * Get category tree with full hierarchy (useful for CMS dropdowns)
   */
      ...
      const buildTree = (parentId: bigint | null = null): any[] => {
        return categories
          .filter((category) => {
            ...
            return category.parent_id?.toString() === parentId.toString();
          })
          .map((category) => ({
            id: Number(category.id),
            name: category.name,
