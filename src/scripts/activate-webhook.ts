...
async function activateWebhook() {
  try {
    ...
    await axios.delete(`${KIOTVIET_BASE_URL}/webhooks/${WEBHOOK_ID}`, {
      headers: {
    ...
    // Tạo webhook mới với isActive = true
    ...
    console.log('New Webhook ID:', response.data.id);
